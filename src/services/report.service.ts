...
export const getPnlData = (
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      const income = Array.from(incomeMap.entries()).map(
        ([category, amount]) => ({
          category,
    ...
    try: async () => {
      ...
      for (const row of rows) {
        ...
        if (loc === "bank" && row.subLocationId) {
          const existing =
            bankAccountBalances[row.subLocationId] ?? new BigNumber(0);
          bankAccountBalances[row.subLocationId] =
            row.txType === "debit"
              ? existing.plus(amount)
              : existing.minus(amount);
        }
      ...
      return {
        ...
        bankAccounts: Object.fromEntries(
          Object.entries(bankAccountBalances).map(([id, bal]) => [
            id,
            formatAmount(bal),
          ]),
        ),
    ...
    try: async () => {
      ...
      if (receivableCat) {
        ...
        for (const row of receivableRows) {
          if (row.type === "debit")
            interestReceivable = interestReceivable.plus(
              new BigNumber(row.amount),
            );
          else
            interestReceivable = interestReceivable.minus(
              new BigNumber(row.amount),
            );
        }
        if (interestReceivable.isLessThan(0))
          interestReceivable = new BigNumber(0);
      }
      ...
      if (payableCat) {
        ...
        for (const row of payableRows) {
          if (row.type === "credit")
            interestPayable = interestPayable.plus(new BigNumber(row.amount));
          else
            interestPayable = interestPayable.minus(new BigNumber(row.amount));
        }
    ...
    try: async () => {
      ...
      for (const { loan, customerName } of activeLoans) {
        ...
        const outstandingBalance =
          ledgerBalance ?? new BigNumber(loan.principalAmount);
...
export const getCashflowData = (
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            inArray(transactionCategories.type, [
              "revenue",
              "expense",
              "asset",
              "liability",
      ...
      function classify(row: (typeof rows)[number]): {
        direction: Direction;
        label: string;
      } {
        const ref = row.referenceType;
        // Real-cash sources keyed off referenceType when set.
        if (ref === "loan_disbursement")
          return { direction: "out", label: "Loan disbursements" };
        if (ref === "payment" || ref === "payment_reversal") {
          ...
          if (row.categoryName === "Cash") {
            return {
              direction: ref === "payment_reversal" ? "out" : "in",
              label: "Loan payments received",
            };
          }
        ...
        if (ref === "creditor_investment") {
          if (row.categoryName === "Cash")
            return { direction: "in", label: "Creditor investments received" };
          return { direction: "skip", label: "" };
        }
        if (ref === "creditor_repayment") {
          if (row.categoryName === "Cash")
            return { direction: "out", label: "Creditor repayments" };
          return { direction: "skip", label: "" };
        }
        if (ref === "fund_transfer" || ref === "capital_injection")
          return { direction: "skip", label: "" };
        if (ref === "interest_accrual" || ref === "penalty_interest_accrual")
          return { direction: "skip", label: "" };
        if (ref === "collateral_settlement")
          return { direction: "skip", label: "" };
      ...
      for (const row of rows) {
        ...
        if (direction === "in") {
          ...
          if (row.monthStart === period) {
            inflowsByType.set(
              label,
              (inflowsByType.get(label) ?? new BigNumber(0)).plus(amt),
            );
          }
        } else {
          ...
          if (row.monthStart === period) {
            outflowsByType.set(
              label,
              (outflowsByType.get(label) ?? new BigNumber(0)).plus(amt),
            );
          }
        }
      }
      ...
      return {
        period,
        months,
        inflowsByType: Array.from(inflowsByType.entries()).map(
          ([label, amt]) => ({ label, amount: formatAmount(amt) }),
        ),
        outflowsByType: Array.from(outflowsByType.entries()).map(
          ([label, amt]) => ({ label, amount: formatAmount(amt) }),
        ),

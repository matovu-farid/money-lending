...
vi.mock("@/services/ledger-queries.service", () => {
  ...
  return {
    ...
    getCreditorTotalInvestedFromLedger: vi
      .fn()
      .mockResolvedValue(new BigNumber(0)),
    getCreditorTotalRepaidFromLedger: vi
      .fn()
      .mockResolvedValue(new BigNumber(0)),
  };
});
...
describe("Creditor Service — DB operations (requires test DB)", () => {
  ...
  beforeEach(async () => {
    ...
    mockedGetCreditorBalancesFromLedger =
      ledgerMod.getCreditorBalancesFromLedger as any;
    mockedGetInterestPayableFromLedger =
      ledgerMod.getInterestPayableFromLedger as any;
    mockedGetCreditorTotalInvestedFromLedger =
      ledgerMod.getCreditorTotalInvestedFromLedger as any;
    mockedGetCreditorTotalRepaidFromLedger =
      ledgerMod.getCreditorTotalRepaidFromLedger as any;
    ...
    const mockTx: any = {
      insert: vi.fn().mockReturnValue({
        values: vi.fn().mockReturnValue({
          returning: vi
            .fn()
            .mockResolvedValue([overrides?.insertResult ?? mockCreditor]),
        }),
      }),
      update: vi.fn().mockReturnValue({
        set: vi.fn().mockReturnValue({
          where: vi.fn().mockReturnValue({
            returning: vi
              .fn()
              .mockResolvedValue([overrides?.updateResult ?? mockCreditor]),
          }),
        }),
      }),
  ...
  it("createCreditor: inserts creditor record and returns Creditor type (CRED-01)", async () => {
    ...
    const result = await Effect.runPromise(
      createCreditor(
        { name: "Alice Fund", contact: "+256700000001", address: "Kampala" },
        "actor-1",
      ),
    );
  ...
  it("getCreditorDashboard: computes interestAccrued using minInterestDays=0 (CRED-03)", async () => {
    ...
    mockedGetCreditorTotalInvestedFromLedger.mockResolvedValueOnce(
      new BigNumber("10000000"),
    );
    // Mock total repaid from ledger
    mockedGetCreditorTotalRepaidFromLedger.mockResolvedValueOnce(
      new BigNumber("0"),
    );

    const result = (await Effect.runPromise(
      getCreditorDashboard("cred-1"),
    )) as any;

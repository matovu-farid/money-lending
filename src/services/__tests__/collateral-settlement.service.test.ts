...
describe("Collateral Settlement Service", () => {
  ...
  it("activePayments query excludes markedWrong payments during collateral settlement", async () => {
            ...
            set: vi.fn().mockReturnValue({
              where: vi.fn().mockReturnValue({
                returning: vi
                  .fn()
                  .mockResolvedValue([
                    { ...mockLoan, status: "settled_with_collateral" },
                  ]),
              }),
            }),
          }),
    ...
    const { settleWithCollateral } =
      await import("@/services/collateral-settlement.service");
    await Effect.runPromise(
      settleWithCollateral(
        { loanId: "loan-1", reason: "Defaulted" },
        "actor-1",
      ),
    );
  ...
  it("settles a loan with collateral: posts interest, principal recovery, seizes collateral, updates status", async () => {
    ...
    const { postJournalEntry, reverseInterestAccrual } =
      await import("@/services/transaction.service");
    const { autoPostPrincipalRecovery } =
      await import("@/services/auto-post.service");
  ...
  it("returns LoanNotFound when loan does not exist", async () => {
    ...
    const exit = await Effect.runPromiseExit(
      settleWithCollateral(
        { loanId: "nonexistent", reason: "Defaulted" },
        "actor-1",
      ),
    );
  ...
  it("returns ValidationError when loan is already settled with collateral", async () => {
    ...
    const exit = await Effect.runPromiseExit(
      settleWithCollateral(
        { loanId: "loan-1", reason: "Defaulted again" },
        "actor-1",
      ),
    );
            ...
            innerJoin: vi.fn().mockReturnValue({
              where: vi.fn().mockReturnValue({
                limit: vi
                  .fn()
                  .mockResolvedValue([
                    { loan: mockLoan, customerName: "Jane Doe" },
                  ]),
              }),
            }),
          }),

...
describe("Report Service — Portfolio risk flag (RPTS-04)", () => {
  it("riskFlag=true when daysOverdue >= 30 (45 days unpaid)", () => {
    ...
    const dailyInterestAmount = new BigNumber(outstandingBalance).multipliedBy(
      dailyRate,
    );
...
describe("Report Service — Snapshot idempotency (RPTS-02 / RPTS-03)", () => {
  ...
  it("generateMonthlySnapshot: inserts pnl and balance_sheet rows with onConflictDoNothing", async () => {
    ...
    const valuesStub = vi
      .fn()
      .mockReturnValue({ onConflictDoNothing: onConflictDoNothingStub });
  ...
  it("getPortfolioData: returns active loans sorted by daysOverdue descending", async () => {
    ...
    const { getLoanBalancesFromLedger, getInterestEarnedFromLedger } =
      await import("@/services/ledger-queries.service");
    ...
    expect(parseInt(result[0].daysOverdue)).toBeGreaterThan(
      parseInt(result[1].daysOverdue),
    );

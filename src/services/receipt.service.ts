...
export type ReceiptResult = TransactionReceiptData | { error: string }
...
async function buildExpenseReceipt(transactionId: string): Promise<ReceiptResult> {
...
async function buildIncomeReceipt(transactionId: string): Promise<ReceiptResult> {
  const [tx] = await db.select().from(transactions).where(eq(transactions.id, transactionId))
  if (!tx) return { error: "Transaction not found" }
  const [actor, bankName] = await Promise.all([
    resolveActorName(tx.recordedBy),
...
async function buildCreditorInvestmentReceipt(investmentId: string): Promise<ReceiptResult> {
...
async function buildCreditorRepaymentReceipt(repaymentId: string): Promise<ReceiptResult> {
...
async function buildFundTransferReceipt(transferId: string): Promise<ReceiptResult> {
...
async function buildSettlementReceipt(loanId: string): Promise<ReceiptResult> {
...
/** Builds normalized receipt data for any money-movement event. */
export async function buildReceipt(input: ReceiptInput): Promise<ReceiptResult> {
  switch (input.kind) {
    case "expense":

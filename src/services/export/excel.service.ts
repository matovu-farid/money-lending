...
export async function generateBalanceSheetExcel(
  ...
  if (parseFloat(data.assets.interestReceivable) > 0) {
    assetRows.push([
      "Interest Receivable",
      parseFloat(data.assets.interestReceivable),
    ]);
  }
  ...
  if (parseFloat(data.assets.seizedCollateralValue) > 0) {
    assetRows.push([
      "Seized Collateral",
      parseFloat(data.assets.seizedCollateralValue),
    ]);
  }
  ...
  const interestPayableVal = parseFloat(
    data.liabilities.interestPayable ?? "0",
  );
  ...
  const scRow = sheet.addRow([
    "Share Capital",
    parseFloat(data.equity.shareCapital),
  ]);
  scRow.getCell(2).numFmt = UGX_FORMAT;
  applyDataRowStyle(scRow, false);

  const reRow = sheet.addRow([
    "Retained Earnings",
    parseFloat(data.equity.retainedEarnings),
  ]);
  reRow.getCell(2).numFmt = UGX_FORMAT;
  applyDataRowStyle(reRow, true);

  const teRow = sheet.addRow([
    "Total Equity",

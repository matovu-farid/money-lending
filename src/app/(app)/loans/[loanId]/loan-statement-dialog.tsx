  ...
  return (
    <DrawerDialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="text-sm font-mono space-y-7 mt-4">
          <HeaderBlock
            loanRef={loanRef}
...
function TermsBlock({ statement }: { statement: LoanStatement }) {
  ...
  return (
    <Section title="Loan Terms">
      ...
      />
      <Row label="Penalty Threshold" value={`${t.penaltyThresholdDays} days overdue`} />
      <Row
    ...
    case "penalty_waived":
      return (
        <div className="border-l-2 border-yellow-500/40 pl-4">
          ...
          {event.waivedBy && <div className="text-muted-foreground mt-1">By: {event.waivedBy}</div>}
        </div>
      )
    case "rate_changed":
      return (
        <div className="border-l-2 border-blue-500/40 pl-4">
          <div className="flex items-baseline justify-between gap-2">
            <span className="font-semibold">{dayLabel} · {dateLabel}</span>
            <span className="text-[10px] uppercase tracking-wider text-blue-600">Rate Changed</span>
          </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {cycles.map((c) => (
              <tr
                key={c.cycle}
                className={cn("border-b border-border/50", c.isPartial && "bg-muted/30 italic")}
              >
                ...
                <td className="py-1.5 pr-3 text-right tabular-nums">{formatCurrency(c.startBalance)}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums">{formatCurrency(c.endBalance)}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums">{c.daysAtBaseRate}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums">{c.daysAtEffectiveRate || "—"}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums">{formatCurrency(c.accruedInCycle)}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums">{formatCurrency(c.cumulativePaid)}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums">{formatCurrency(c.netUnpaidAtEnd)}</td>
                <td className="py-1.5 text-right tabular-nums">
                  {c.daysOverdueAtEnd}d
                  {c.penaltyActiveAtEnd && <span className="ml-1 text-destructive">⚠</span>}
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    <Section title={`As Of ${format(statement.today, "MMM d, yyyy")}`}>
      ...
      <Row
        ...
        value={<span className="text-lg font-bold">{formatCurrency(f.totalDue)}</span>}
      />

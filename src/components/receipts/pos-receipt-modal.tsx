...
export function PosReceiptModal({
  ...
  const receiptImageRef = useRef<string | null>(null);
  const generateReceiptImage = useCallback(async () => {
    if (receiptImageRef.current) {
      return receiptImageRef.current;
    }

    const receiptEl = receiptRef.current?.querySelector(
      ".pos-receipt",
    ) as HTMLElement | null;
    ...
    receiptImageRef.current = dataUrl;
  ...
  return (
    ...
    >
      <DialogContent
        ...
        className="sm:max-w-[380px] max-h-[90vh] overflow-y-auto"
      >
        <DialogHeader>

...
function randomRate(): string {
  ...
  return randomDecimal(0.01, 0.30, 4)
}
...
describe("FUZZ: Payment Allocation - Conservation of Money", () => {
  it("interest + principal = payment amount (perpetual, payment <= total owed)", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      const reconstructed = new BigNumber(allocation.interestPortion)
        .plus(new BigNumber(allocation.principalPortion))
  ...
  it("principal balance never goes negative", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      expect(
        new BigNumber(allocation.principalBalanceAfter).isGreaterThanOrEqualTo(0),
  ...
  it("principalBalanceAfter = principalBalanceBefore - principalPortion", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      const expected = BigNumber.max(
        new BigNumber(principal).minus(new BigNumber(allocation.principalPortion)),
        0
      )
      const diff = new BigNumber(allocation.principalBalanceAfter).minus(expected).abs()
      expect(
        diff.isLessThanOrEqualTo(1),
...
describe("FUZZ: Interest-First Rule", () => {
  it("small payments go entirely to interest (no principal reduction)", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      const rate = randomDecimal(0.05, 0.20, 4)
      const days = randomInt(30, 180)
      ...
      const smallPayment = String(randomInt(1, Math.max(interest.minus(1).integerValue().toNumber(), 1)))
...
describe("FUZZ: Overdue Calculation Invariants", () => {
  it("fully paid loan (interestPaid >= interestAccrued) has 0 days overdue", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      const totalInterest = new BigNumber(principal).multipliedBy(dailyRate).multipliedBy(daysElapsed)
  ...
  it("effective rate >= base rate always", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      expect(new BigNumber(effective).isGreaterThanOrEqualTo(new BigNumber(rate))).toBe(true)
    }
  })
})
...
describe("FUZZ: Fixed Rate Schedule Invariants", () => {
  ...
  it("final balance is zero (principal fully amortized)", () => {
    for (let i = 0; i < 50; i++) {
      ...
      expect(new BigNumber(lastEntry.balanceAfter).abs().isLessThanOrEqualTo(1)).toBe(true)
    }
  })
...
describe("FUZZ: Multiple Payments - interestAlreadyPaidInPeriod", () => {
  it("two payments in same period: total interest <= single-payment interest", () => {
    for (let i = 0; i < FUZZ_ITERATIONS; i++) {
      ...
      const firstAmount = String(randomInt(1, Math.max(Number(totalOwed.toFixed(0)) - 1, 1)))
      ...
      const secondAmount = String(
        Math.min(
          ...
          Number(new BigNumber(firstAlloc.principalBalanceAfter)
      ...
      const totalInterest = new BigNumber(firstAlloc.interestPortion)
        .plus(new BigNumber(secondAlloc.interestPortion))
      ...
      expect(
        totalInterest.isLessThanOrEqualTo(new BigNumber(singleAlloc.interestPortion).plus(2)),
  ...
  it("N sequential payments never violate conservation of money", () => {
    for (let trial = 0; trial < 50; trial++) {
      ...
      for (let p = 0; p < numPayments; p++) {
        ...
        totalInterestPaid = totalInterestPaid.plus(new BigNumber(allocation.interestPortion))
        totalPrincipalPaid = totalPrincipalPaid.plus(new BigNumber(allocation.principalPortion))
      ...
      expect(
        totalPrincipalPaid.isLessThanOrEqualTo(new BigNumber(principal).plus(1)),
...
describe("FUZZ: formatAmount Rounding Consistency", () => {
  ...
  it("rounding error accumulation across payments stays bounded", () => {
    ...
    for (let day = 1; day <= 365; day++) {
      ...
      accumulatedError = accumulatedError.plus(exactInterest.minus(roundedInterest))
    }

...
const arbRate = fc.integer({ min: 100, max: 3000 }).map((n) => (n / 10000).toFixed(4))
...
const arbEdgeDate = fc.oneof(
  ...
  fc.constantFrom(0, 2, 4, 6, 7, 9, 11).chain((month) =>
    fc.integer({ min: 2020, max: 2027 }).map((year) => new Date(year, month, 31))
  ),
)
...
describe("L1: Interest Calculation Properties", () => {
  it("interest is always non-negative", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, arbDays, arbMinDays, (principal, rate, days, minDays) => {
    ...
    fc.assert(
      fc.property(
        ...
        (rate, days, basePrincipal, extraPrincipal) => {
          ...
          const i2 = calculateInterest(String(basePrincipal + extraPrincipal), rate, days, 0)
          return i2.isGreaterThanOrEqualTo(i1)
        }
      ),
      { numRuns: 500 }
  ...
  it("zero rate yields zero interest", () => {
    fc.assert(
      fc.property(arbPrincipal, arbDays, arbMinDays, (principal, days, minDays) => {
  ...
  it("daily rate × 30 = monthly rate (exact)", () => {
    fc.assert(
      fc.property(arbRate, (rate) => {
        ...
        return reconstructed.minus(new BigNumber(rate)).abs().isLessThan(0.0000001)
      }),
      { numRuns: 200 }
    )
  })
...
describe("L1: Payment Allocation Properties", () => {
  it("conservation: interest + principal = payment (within rounding)", () => {
    fc.assert(
      fc.property(arbLoanParams, fc.integer({ min: 30, max: 365 }), (params, days) => {
        const interest = calculateInterest(params.principal, params.rate, days, params.minDays)
        ...
        const paymentAmount = totalOwed.multipliedBy(paymentFraction).integerValue().toFixed(0)
        ...
        return sum.minus(new BigNumber(paymentAmount)).abs().isLessThanOrEqualTo(1)
      }),
      { numRuns: 500 }
    )
  })
    ...
    fc.assert(
      fc.property(
        ...
        (params, days) => {
          ...
          return new BigNumber(alloc.principalBalanceAfter).isGreaterThanOrEqualTo(0)
        }
      ),
      { numRuns: 500 }
    )
...
describe("L1: Amortization Schedule Properties", () => {
  it("fixed_rate: interest is constant every month", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, fc.integer({ min: 2, max: 24 }), (principal, rate, term) => {
        const { entries } = calculateSchedule(principal, rate, term, "fixed_rate")
  ...
  it("reducing_balance: interest is non-increasing", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, fc.integer({ min: 2, max: 24 }), (principal, rate, term) => {
        const { entries } = calculateSchedule(principal, rate, term, "reducing_balance")
        for (let i = 1; i < entries.length; i++) {
          if (new BigNumber(entries[i].monthlyInterest).isGreaterThan(
  ...
  it("reducing_balance total interest <= fixed_rate total interest", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, fc.integer({ min: 2, max: 24 }), (principal, rate, term) => {
        ...
        const reducing = calculateSchedule(principal, rate, term, "reducing_balance")
        return reducing.totalInterest.isLessThanOrEqualTo(fixed.totalInterest.plus(term)) // rounding tolerance
      }),
      { numRuns: 200 }
    )
  })
  ...
  it("final balance is zero after full schedule", () => {
    fc.assert(
      fc.property(
        ...
        (principal, rate, term, type) => {
          const { entries } = calculateSchedule(principal, rate, term, type)
          return new BigNumber(entries[entries.length - 1].balanceAfter).abs().isLessThanOrEqualTo(1)
        }
      ),
      { numRuns: 200 }
    )
  ...
  it("conservation: interest + principal = payment (within rounding)", () => {
    fc.assert(
      fc.property(
        ...
        arbTerm,
        (principal, rate, term) => {
          const paymentNumber = fc.sample(fc.integer({ min: 1, max: term }), 1)[0]
          const monthlyInterest = new BigNumber(principal).multipliedBy(new BigNumber(rate))
          const totalOwed = monthlyInterest.plus(new BigNumber(principal))
          const paymentAmount = totalOwed.multipliedBy(Math.random()).integerValue().toFixed(0)
          if (Number(paymentAmount) <= 0) return true
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const monthlyInterest = new BigNumber(principal).multipliedBy(new BigNumber(rate))
          const paymentAmount = monthlyInterest.plus(new BigNumber(principal)).multipliedBy(2).integerValue().toFixed(0)
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const halfInterest = monthlyInterest.dividedBy(2).integerValue().toFixed(0)
          if (Number(halfInterest) <= 0) return true
          ...
          return alloc.principalPortion === "0.00" &&
            alloc.principalBalanceAfter === formatAmount(new BigNumber(principal))
        }
      ),
      { numRuns: 300 }
    )
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const totalRemainingInterest = monthlyInterest.multipliedBy(remainingMonths)
          const balance = new BigNumber(principal)

          // Pay enough to trigger early payoff (all remaining interest + full balance)
          const paymentAmount = totalRemainingInterest.plus(balance).integerValue().toFixed(0)
          ...
          return new BigNumber(alloc.interestPortion).minus(new BigNumber(expectedInterest)).abs().isLessThanOrEqualTo(1)
        }
      ),
      { numRuns: 300 }
    )
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const alreadyPaid = monthlyInterest.dividedBy(2).integerValue().toFixed(0)
          // Pay less than monthlyInterest to stay in normal (non-early-payoff) path
          const paymentAmount = monthlyInterest.dividedBy(3).integerValue().toFixed(0)
          if (Number(paymentAmount) <= 0) return true
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const paymentAmount = totalRemainingInterest.plus(balance).plus(1000).integerValue().toFixed(0)
  ...
  it("conservation: interest + principal = payment (within rounding)", () => {
    fc.assert(
      fc.property(
        ...
        arbTerm,
        (principal, rate, term) => {
          const interest = new BigNumber(principal).multipliedBy(new BigNumber(rate))
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const interest = new BigNumber(principal).multipliedBy(new BigNumber(rate))
          const paymentAmount = interest.plus(new BigNumber(principal)).multipliedBy(2).integerValue().toFixed(0)
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          const currentBalance = new BigNumber(principal).dividedBy(2).integerValue().toFixed(0)
          const paymentNumber = fc.sample(fc.integer({ min: 1, max: term }), 1)[0]

          // Pay just the interest portion to isolate the interest calculation
          const reducingInterest = new BigNumber(currentBalance).multipliedBy(new BigNumber(rate))
          const fixedInterest = new BigNumber(principal).multipliedBy(new BigNumber(rate))
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          // Pay more than enough to cover everything
          const paymentAmount = interest.plus(balance).plus(1000).integerValue().toFixed(0)
...
function simulateLoanLifecycle(params: {
  ...
  const { principal, rate, minDays, startDate, payments: paymentSchedule } = params
  ...
  for (const paymentDef of paymentSchedule) {
    ...
    const paymentBN = totalOwed.multipliedBy(paymentDef.fractionOfOwed).integerValue()
    ...
    if (new BigNumber(alloc.principalBalanceAfter).isLessThan(0)) {
      violations.push(`Payment #${paymentNumber}: negative balance ${alloc.principalBalanceAfter}`)
    }

    // 3. Balance consistency
    const expectedBalance = BigNumber.max(balance.minus(new BigNumber(alloc.principalPortion)), 0)
    if (new BigNumber(alloc.principalBalanceAfter).minus(expectedBalance).abs().isGreaterThan(1)) {
      violations.push(
    ...
    if (new BigNumber(paymentAmount).isLessThanOrEqualTo(interest) && new BigNumber(alloc.principalPortion).isGreaterThan(1)) {
      violations.push(
    ...
    totalInterestPaid = totalInterestPaid.plus(new BigNumber(alloc.interestPortion))
...
describe("L2: Loan Lifecycle Simulation", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (principal, rate, minDays, startDate, payments) => {
          const result = simulateLoanLifecycle({ principal, rate, minDays, startDate, payments })
  ...
  it("single large payment fully pays loan", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, arbDate, (principal, rate, startDate) => {
  ...
  it("many tiny payments converge to full repayment without invariant violations", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, arbDate, (principal, rate, startDate) => {
        ...
        const result = simulateLoanLifecycle({ principal, rate, minDays: 30, startDate, payments })
        return result.violations.length === 0
      }),
      { numRuns: 200 }
    )
...
function simulateTermLoanLifecycle(params: {
  ...
  const violations: string[] = []
  const { principal, rate, termMonths, loanType, payments: paymentSchedule } = params
  ...
  for (const paymentDef of paymentSchedule) {
    ...
    const rawPayment = monthlyInstallment.multipliedBy(paymentDef.fractionOfMonthlyInstallment).integerValue()
    ...
    if (new BigNumber(paymentAmount).isLessThanOrEqualTo(interestThisPeriod) && new BigNumber(alloc.principalPortion).isGreaterThan(1)) {
      violations.push(
    ...
    totalPrincipalPaid = totalPrincipalPaid.plus(new BigNumber(alloc.principalPortion))
    if (totalPrincipalPaid.isGreaterThan(originalPrincipal.plus(paymentNumber + 1))) {
      violations.push(
    ...
    if (loanType === "fixed_rate" && !alloc.loanFullyPaid) {
      ...
      const isEarlyPayoff = new BigNumber(paymentAmount).isGreaterThanOrEqualTo(earlyPayoffThreshold)
      // Only check when payment covers the full interest and is not an early payoff
      if (!isEarlyPayoff && new BigNumber(paymentAmount).isGreaterThan(expectedInterest) && actualInterest.minus(expectedInterest).abs().isGreaterThan(1)) {
        violations.push(
    ...
    if (loanType === "reducing_balance" && new BigNumber(alloc.principalPortion).isGreaterThan(0)) {
      const actualInterest = new BigNumber(alloc.interestPortion)
...
describe("L2: Term Loan Lifecycle Simulation", () => {
  const arbPaymentFraction = fc.record({
    fractionOfMonthlyInstallment: fc.double({ min: 0.1, max: 2.0, noNaN: true }),
  })
  ...
  it("fixed_rate: exact monthly installments fully pay loan", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, arbTerm, (principal, rate, termMonths) => {
  ...
  it("fixed_rate: early payoff charges all remaining term interest", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, arbTerm, (principal, rate, termMonths) => {
        const originalPrincipal = new BigNumber(principal)
        const monthlyInterest = originalPrincipal.multipliedBy(new BigNumber(rate))
        // One massive payment to pay everything at once (round up to avoid threshold rounding issues)
        const totalNeeded = originalPrincipal.plus(monthlyInterest.multipliedBy(termMonths))
        const alloc = allocateFixedRatePayment({
          paymentAmount: totalNeeded.integerValue(BigNumber.ROUND_CEIL).toFixed(0),
          principalBalanceBefore: principal,
          originalPrincipal: principal,
          monthlyRateDecimal: rate,
          termMonths,
        ...
        const expectedTotalInterest = monthlyInterest.multipliedBy(termMonths)
        const actualInterest = new BigNumber(alloc.interestPortion)
        expect(actualInterest.minus(expectedTotalInterest).abs().isLessThanOrEqualTo(1)).toBe(true)
      }),
      { numRuns: 200 }
    )
  })
  ...
  it("reducing_balance: early payoff only charges current-balance interest", () => {
    fc.assert(
      fc.property(arbPrincipal, arbRate, arbTerm, (principal, rate, termMonths) => {
        ...
        const actualInterest = new BigNumber(alloc.interestPortion)
        expect(actualInterest.minus(reducingInterest).abs().isLessThanOrEqualTo(1)).toBe(true)

        // Reducing balance early payoff interest should be <= fixed_rate early payoff interest
        const fixedTotalInterest = originalPrincipal.multipliedBy(monthlyRate).multipliedBy(termMonths)
        expect(actualInterest.isLessThanOrEqualTo(fixedTotalInterest.plus(1))).toBe(true)
      }),
      { numRuns: 200 }
    )
  })
      ...
      fc.property(
        ...
        (principal, rate, totalDays, paymentDefs) => {
          ...
          for (const def of paymentDefs) {
            ...
            const portion = runningBalance.multipliedBy(def.fractionRepaid).integerValue()
...
describe("L3: Time-Warp — Overdue at Dangerous Dates", () => {
  ...
  for (const { name, date } of dangerousDates) {
    ...
    it(`term loan starting on ${name}: month counting correct`, () => {
      fc.assert(
        fc.property(arbPrincipal, arbRate, fc.integer({ min: 3, max: 12 }), (principal, rate, term) => {
  ...
  it("periodBoundsUTC always produces valid UTC boundaries", () => {
    fc.assert(
      fc.property(arbPeriod, (period) => {
        ...
        if (periodStart.getUTCHours() !== 0 || periodStart.getUTCMinutes() !== 0) return false
        if (periodStart.getUTCDate() !== 1) return false

        // End must be 23:59:59.999 UTC on last day
        if (periodEnd.getUTCHours() !== 23 || periodEnd.getUTCMinutes() !== 59) return false
        if (periodEnd.getUTCSeconds() !== 59 || periodEnd.getUTCMilliseconds() !== 999) return false
...
describe("L3: daysBetween Calendar Properties", () => {
  ...
  it("daysBetween is non-negative when to >= from", () => {
    fc.assert(
      fc.property(arbDate, fc.integer({ min: 0, max: 730 }), (from, daysToAdd) => {
    ...
    fc.assert(
      fc.property(
        ...
        (from, baseDays, extraDays) => {
          ...
          const to2 = new Date(from.getTime() + (baseDays + extraDays) * 86400000)
  ...
  it("month-end edge dates produce correct day counts", () => {
    fc.assert(
      fc.property(arbEdgeDate, fc.integer({ min: 1, max: 90 }), (startDate, daysToAdd) => {
  ...
  it("effective rate >= base rate always", () => {
    fc.assert(
      fc.property(arbRate, fc.boolean(), (rate, penaltyActive) => {
        ...
        return new BigNumber(effective).isGreaterThanOrEqualTo(new BigNumber(rate))
      }),
      { numRuns: 200 }
    )
  })

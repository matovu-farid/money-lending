...
function checkInvariants(model: LoanModel): string[] {
  ...
  if (model.totalPrincipalPaid.isGreaterThan(new BigNumber(model.principal).plus(1))) {
    violations.push(
  ...
  if (info.penaltyActive && info.daysOverdue < 60) {
    violations.push(`Penalty active at ${info.daysOverdue} days (should be >= 60)`)
  }
...
class MakePaymentCommand implements fc.Command<LoanModel, RealState> {
  ...
  run(model: LoanModel, _real: RealState): void {
    ...
    const activePayments = model.payments.filter((p) => !p.deleted)
    const prevDate = activePayments.length === 0
      ? model.startDate
      : activePayments[activePayments.length - 1].date
    const daysElapsed = daysBetween(prevDate, paymentDate)

    // Calculate what's owed
    const interest = calculateInterest(model.balance.toFixed(0), model.rate, daysElapsed, model.minDays)
    ...
    model.totalInterestPaid = model.totalInterestPaid.plus(new BigNumber(alloc.interestPortion))
    model.totalPrincipalPaid = model.totalPrincipalPaid.plus(new BigNumber(alloc.principalPortion))
    ...
    if (violations.length > 0) {
      throw new Error(`MakePayment(days=${this.daysFromCurrent}, fraction=${this.fractionOfOwed}):\n${violations.join("\n")}`)
    }
  }
...
class DeletePaymentCommand implements fc.Command<LoanModel, RealState> {
  ...
  run(model: LoanModel, _real: RealState): void {
    ...
    model.totalInterestPaid = model.totalInterestPaid.minus(new BigNumber(payment.interestPortion))
    model.totalPrincipalPaid = model.totalPrincipalPaid.minus(new BigNumber(payment.principalPortion))
    model.balance = model.balance.plus(new BigNumber(payment.principalPortion))

    // Clamp negatives from rounding
    if (model.totalInterestPaid.isLessThan(0)) model.totalInterestPaid = new BigNumber(0)
    if (model.totalPrincipalPaid.isLessThan(0)) model.totalPrincipalPaid = new BigNumber(0)

    const violations = checkInvariants(model)
    if (violations.length > 0) {
      throw new Error(`DeletePayment(index=${this.paymentIndex}):\n${violations.join("\n")}`)
    }
  }
...
class AdvanceTimeCommand implements fc.Command<LoanModel, RealState> {
  ...
  run(model: LoanModel, _real: RealState): void {
    ...
    const violations = checkInvariants(model)
    if (violations.length > 0) {
      throw new Error(`AdvanceTime(days=${this.days}):\n${violations.join("\n")}`)
    }
  }
...
const makePaymentCmd = fc.record({
...
const deletePaymentCmd = fc.integer({ min: 0, max: 10 }).map(
  (idx) => new DeletePaymentCommand(idx)
)

const advanceTimeCmd = fc.integer({ min: 1, max: 90 }).map(
  (days) => new AdvanceTimeCommand(days)
)
      ...
      fc.property(
        ...
        (principal, rate, term) => {
          ...
          for (let month = 1; month <= term; month++) {
            ...
            totalInterest = totalInterest.plus(new BigNumber(alloc.interestPortion))
            totalPrincipal = totalPrincipal.plus(new BigNumber(alloc.principalPortion))
            balance = new BigNumber(alloc.principalBalanceAfter)

            // Balance never negative
            if (balance.isLessThan(0)) {
              throw new Error(`Month ${month}: negative balance ${balance.toFixed(0)}`)
            }
          }

          // After all payments, balance should be zero or near-zero
          if (balance.isGreaterThan(term + 1)) {
            throw new Error(`After ${term} payments: balance ${balance.toFixed(0)} (expected ~0)`)
          }

          // Total principal paid should approximately equal original
          if (totalPrincipal.minus(new BigNumber(principal)).abs().isGreaterThan(term + 1)) {
            throw new Error(

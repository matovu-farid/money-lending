...
async function main() {
  ...
  if (APPLY) {
    console.log(
      `[backfill] adjustments will be recorded by user ${sysUser.email} (${recordedBy})`,
    );
  }
  ...
  for (const r of reports) {
    console.log(
      `loan ${r.loanId} — principal ${r.principalAmount}, ${r.paymentCount} payments, over-charged UGX ${r.totalOverCharged}`,
    );
  ...
  if (!APPLY) {
    console.log();
    console.log(
      "[backfill] dry run complete — no changes posted. Re-run with --apply to commit.",
    );
    process.exit(0);
  }
  ...
  console.log();
  console.log(
    `[backfill] applied — ${affectedPayments} adjustments across ${reports.length} loans, total UGX ${grandTotal.toFixed(2)}`,
  );
}
